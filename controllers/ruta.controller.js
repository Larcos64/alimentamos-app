...
exports.listar = async (req, res) => {
    try {
        const [rutas, ciudades, conductores] = await Promise.all([
            db.query(`
            SELECT R.*, 
                ...
                COALESCE(CON.nombres || ' ' || CON.apellidos, 'Sin asignar') AS nombre_conductor
            FROM RUTA R
            ...
            LEFT JOIN CONDUCTOR CON ON R.id_conductor = CON.id
        `),
            db.query('SELECT * FROM CIUDAD'),
            conductorController.getConductores()
        ]);
...
exports.guardar = async (req, res) => {
    try {
        ...
        const rutaResult = await db.query(
            `INSERT INTO RUTA (nombre, cod_ciudad_origen, cod_ciudad_destino, fecha_apertura, id_conductor)
            VALUES ($1, $2, $3, $4, $5) RETURNING id`,
            [nombre, cod_ciudad_origen, cod_ciudad_destino, fechaValida, id_conductor || null]
        );
...
exports.editar = async (req, res) => {
    try {
        ...
        SET nombre = $1,
            ...
            id_conductor = $5
        WHERE id = $6
    `, [nombre, cod_ciudad_origen, cod_ciudad_destino, fechaValida, id_conductor || null, id]);
